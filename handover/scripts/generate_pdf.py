...
def build():
    ...
    story += [table(["Kiểm tra", "Kết quả", "Ghi chú"], [("Frontend Vitest", "84/84 pass", "27 test files"), ("Frontend build", "Pass", "JS ~1.472 MB; gzip 427 kB"), ("Backend Maven", "39/39 pass", "12 suites"), ("E2E thật", "Chưa chạy", "Cần SQL Server + frontend/backend")], [50*mm, 35*mm, 73*mm]), Spacer(1, 5*mm)]
    for text in ["Tối ưu bundle bằng route lazy-loading/code splitting.", "Service worker hiện chỉ dọn worker/cache PWA cũ; không hỗ trợ offline.", "Chạy Playwright và smoke test đầy đủ trên SQL Server demo.", "Kiểm chứng OAuth, SMTP, Redis, upload và WebSocket sau reverse proxy production.", "Dọn cảnh báo Redis/JPA repository scanning và H2 dialect trong test.", "Ảnh Drive có bảng nguồn; ảnh hero AI cần metadata và xác nhận quyền.", "Review working tree, commit và push có chủ đích; hiện chưa thể xác nhận code mới nhất đã lên remote."]:
        story.append(bullet(text))
