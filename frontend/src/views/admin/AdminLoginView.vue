<script setup>
import { ref } from 'vue'
import { useRoute, useRouter } from 'vue-router'
import { useI18n } from 'vue-i18n'
import { loginAdmin, setAuthProvider } from '../../services/authService'
import { Chrome, Lock, Mail, ShieldAlert } from 'lucide-vue-next'
import { googleLoginUrl } from '../../services/oauthService'
import Navbar from '../../components/public/Navbar.vue'
import Footer from '../../components/public/Footer.vue'

const router = useRouter()
const route = useRoute()
const { t } = useI18n()
const email = ref('')
const password = ref('')
const errorMessage = ref('')
const isSubmitting = ref(false)

const handleGoogleLogin = () => {
  window.location.assign(googleLoginUrl())
}

const resolvePostLoginPath = (user) => {
  const redirect = route.query.redirect
  if (typeof redirect === 'string' && redirect.startsWith('/') && !redirect.startsWith('//')) {
    if (user.role === 'ADMIN' && redirect.startsWith('/admin')) return redirect
    if (user.role !== 'ADMIN' && !redirect.startsWith('/admin')) return redirect
  }
  if (user.role !== 'ADMIN') return '/account'
  return '/admin'
}

const handleLogin = async () => {
  if (isSubmitting.value) return

  errorMessage.value = ''
  isSubmitting.value = true

  try {
    const { data } = await loginAdmin({
      email: email.value,
      password: password.value,
    })

    const user = data.user || {}
    localStorage.setItem('admin_token', data.token)
    localStorage.setItem('admin_user', JSON.stringify(user))
    setAuthProvider('local')
    window.dispatchEvent(new Event('aloo-auth-change'))
    router.push(resolvePostLoginPath(user))
  } catch (error) {
    errorMessage.value = error.response?.data?.message || t('admin.login.invalidCredentials')
  } finally {
    isSubmitting.value = false
  }
}
</script>

<template>
  <div>
    <Navbar />
    <main class="grid min-h-screen place-items-center bg-brand-cream px-4 relative overflow-hidden pt-16">
    <!-- Visual background blobs -->
    <div class="absolute -left-20 -top-20 w-96 h-96 bg-avocado-200/20 rounded-full blur-3xl pointer-events-none"></div>
    <div class="absolute -right-20 -bottom-20 w-96 h-96 bg-cream-300/30 rounded-full blur-3xl pointer-events-none"></div>

    <div class="w-full max-w-md relative z-10">
      <form class="rounded-[2rem] bg-white p-8 sm:p-10 shadow-xl border border-avocado-100/20" @submit.prevent="handleLogin">
        <!-- Logo -->
        <div class="flex items-center gap-2 mb-6">
          <div class="h-9 w-9 bg-cream-400 text-avocado-950 font-black rounded-xl grid place-items-center text-sm shadow-md">
            A
          </div>
          <div>
            <div class="text-xl font-bold tracking-tight text-avocado-950">{{ t('admin.login.brandTitle') }}</div>
            <p class="text-[10px] uppercase font-bold tracking-widest text-slate-400">{{ t('admin.login.brandSubtitle') }}</p>
          </div>
        </div>

        <h1 class="text-2xl font-black text-avocado-950 leading-tight">{{ t('admin.login.title') }}</h1>

        <!-- Error panel -->
        <div
          v-if="errorMessage"
          class="mt-5 rounded-2xl bg-red-50 border border-red-200/50 px-4 py-3 text-xs font-bold text-red-700 flex items-center gap-2"
          role="alert"
        >
          <ShieldAlert class="h-4 w-4 shrink-0 text-red-600" />
          <span>{{ errorMessage }}</span>
        </div>

        <div class="mt-8 space-y-5">
          <button
            type="button"
            class="flex w-full items-center justify-center gap-2 rounded-full border border-slate-100 bg-white px-6 py-4 text-xs font-bold uppercase tracking-wider text-slate-700 transition hover:border-avocado-200 hover:bg-avocado-50"
            @click="handleGoogleLogin"
          >
            <Chrome class="h-4.5 w-4.5 text-avocado-700" />
            {{ t('admin.login.google') }}
          </button>

          <div class="flex items-center gap-3 text-[10px] font-bold uppercase tracking-widest text-slate-300">
            <span class="h-px flex-1 bg-slate-100"></span>
            {{ t('admin.login.divider') }}
            <span class="h-px flex-1 bg-slate-100"></span>
          </div>

          <label class="grid gap-2 text-xs font-bold uppercase tracking-wider text-slate-500">
            {{ t('admin.login.email') }}
            <div class="relative">
              <Mail class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 h-4.5 w-4.5" />
              <input v-model="email" type="email" required autocomplete="username" class="w-full rounded-2xl border border-slate-100 bg-slate-50/50 pl-11 pr-4 py-3.5 text-sm font-semibold text-slate-800 outline-none focus:bg-white focus:border-avocado-500 focus:ring-2 focus:ring-avocado-100 transition" />
            </div>
          </label>

          <label class="grid gap-2 text-xs font-bold uppercase tracking-wider text-slate-500">
            {{ t('admin.login.password') }}
            <div class="relative">
              <Lock class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 h-4.5 w-4.5" />
              <input v-model="password" type="password" required autocomplete="current-password" class="w-full rounded-2xl border border-slate-100 bg-slate-50/50 pl-11 pr-4 py-3.5 text-sm font-semibold text-slate-800 outline-none focus:bg-white focus:border-avocado-500 focus:ring-2 focus:ring-avocado-100 transition" />
            </div>
          </label>

          <div class="pt-2">
            <button 
              class="w-full rounded-full bg-avocado-600 px-6 py-4 text-xs font-bold uppercase tracking-wider text-white hover:bg-avocado-700 transition duration-300 disabled:cursor-not-allowed disabled:opacity-60 flex items-center justify-center gap-2 shadow-lg shadow-avocado-600/10" 
              :disabled="isSubmitting"
            >
              {{ isSubmitting ? t('admin.login.submitting') : t('admin.login.submit') }}
            </button>
          </div>
        </div>
      </form>
    </div>
  </main>
  <Footer />
  </div>
</template>
