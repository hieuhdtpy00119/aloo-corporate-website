...
export const getStoredAdminScopes = (storage = localStorage) => {
  ...
  const profile = String(user?.adminProfile || 'CONTENT').toUpperCase()
  return PROFILE_SCOPE_MAP[profile] || PROFILE_SCOPE_MAP.CONTENT
}

export const getAdminPermissions = (role, adminProfile = 'CONTENT', scopes = null) => {
  ...
  const profile = String(adminProfile || 'CONTENT').toUpperCase()
  return scopeListToPermissions(PROFILE_SCOPE_MAP[profile] || PROFILE_SCOPE_MAP.CONTENT)
}
